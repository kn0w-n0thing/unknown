...
ofJson emotionProcessor::processEmotions(string language) {
    ...
    if (!emoJson["frames"].empty() && frames > 0) {
        ...
        obj["average-emotions"]["angry"] = roundf((angry / frames) * divisor) / divisor;
        obj["average-emotions"]["disgusted"] = roundf((disgusted / frames) * divisor) / divisor;
        obj["average-emotions"]["afraid"] = roundf((afraid / frames) * divisor) / divisor;
        obj["average-emotions"]["happy"] = roundf((happy / frames) * divisor) / divisor;
        obj["average-emotions"]["sad"] = roundf((sad / frames) * divisor) / divisor;
        obj["average-emotions"]["surprised"] = roundf((surprised / frames) * divisor) / divisor;
        obj["average-emotions"]["neutral"] = roundf((neutral / frames) * divisor) / divisor;
