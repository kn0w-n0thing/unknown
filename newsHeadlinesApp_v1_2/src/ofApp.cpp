...
void ofApp::setup() {
  ...
  // GET MY IP ADDRESS
  string absolutePath =
      "python3 " + ofToString(ofFilePath::getAbsolutePath(ofToDataPath(""))) +
      "scripts/getIpAddress.py";
  myIpAddressTmp = ofSystem(absolutePath.c_str());
  ...
  debugParam.add(myIpAddress.set("My IP:", myIpAddressTmp));
  ...
  // LOG //
  /////////
  ofLog(OF_LOG_NOTICE, "Absolute path to APP: " + absolutePath);
  ofLog(OF_LOG_NOTICE, "PC-ID: " + ofToString(id));
  ofLog(OF_LOG_NOTICE, "My IP Address: " + myIpAddressTmp);
...
void ofApp::update() {
  ...
  if (!hide) {
    ...
    myIpAddress.set("My IP:", myIpAddressTmp);
  }
}
